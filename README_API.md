...
VLLM_USE_V1_PATH=true  # Se false, não adiciona /v1 à URL base (padrão: true)
...
### Erro 404 (NotFoundError)

Se você receber um erro `openai.NotFoundError: Error code: 404`, isso geralmente indica que o endpoint do vLLM não foi encontrado.

**Soluções:**

1. **Verificar a URL do vLLM**: Teste se a URL está acessível:
   ```bash
   curl https://seu-vllm-url.com/health
   ```

2. **Ajustar o caminho `/v1`**: Se o vLLM não usa `/v1` na URL base, configure:
   ```bash
   VLLM_USE_V1_PATH=false
   ```

3. **Verificar logs**: A API agora inclui logs detalhados mostrando a URL exata que está sendo chamada.

4. **Verificar resposta de erro**: A API retorna detalhes sobre a URL que está tentando acessar.
...
- O caminho `/v1` está correto (use `VLLM_USE_V1_PATH` se necessário)
